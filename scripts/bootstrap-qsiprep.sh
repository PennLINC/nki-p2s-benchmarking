...
PROJECTROOT=${PWD}/qsiprep-${denoise}
if [[ -d ${PROJECTROOT} ]]
then
...
# Remove all the sessions that are not part of the trt
non_trt_sessions=$(find inputs/data/${subid} -maxdepth 1 -name 'ses-*' | grep -v TRT | grep -v $refses)
if [ ! -z "${non_trt_sessions}" ]; then
        rm -rf $non_trt_sessions
fi
...
datalad run \
    ...
    "bash ./code/qsiprep_zip.sh ${subid} ${denoise}"
...
while read line; do
    ...
    ${dssource} ${pushgitremote} ${subject} ${session} ${denoise}" >> code/qsub_calls.sh
done < /cbica/projects/RBC/nki-p2s-benchmarking/trt_closest.txt
